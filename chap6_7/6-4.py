...
def longestPalindrom(s):
    ...
    for i in range(0, len(s)):
        ...
        for j in range(i + 2, len(s)):
            if s[left] == s[j]:
                oddpal.append(s[left:j + 1])
                left -= 1
                if left < 0:
                    break
    print(evenpal)
    print(oddpal)
    temp=evenpal.index(max(evenpal))
    temp2=oddpal.index(max(oddpal))
    print(temp)
    print(temp2)
    ...
    #
    # evenpal = [i for i in evenpal if i not in remove_set]  #[aa, bbbb, cccccc]
    # oddpal = [i for i in oddpal if i not in remove_set]  #[aaa,bbbbb,ccccccc]
